...
# -*- coding: utf-8 -*-
"""
3D-IntelliScan Web Interface

This module provides a Streamlit-based web interface for the 3D-IntelliScan
semiconductor metrology and defect detection pipeline. It offers interactive
visualization of detection results, 3D segmentation, and metrology analysis.

Features:
- Interactive 3D visualization using PyVista
- Real-time pipeline simulation
- PDF report generation and download
- Support for multiple sample analysis

Author: Wang Jie
Date: 1st Aug 2025
"""

from _version import get_version, get_full_info
...
def simulate_object_detection(output_path, progress_callback=None):
    ...
    return os.path.join(output_path, "view1", "detections", "visualize"), os.path.join(
        output_path, "view2", "detections", "visualize"
    )
...
def simulate_3d_segmentation(output_path, bb2d_config, progress_callback=None):
    ...
    if not good_seg_file or not bad_seg_file:
        raise ValueError("Both 'good' and 'bad' segmentation files must be available in the sample directory")
...
def load_bb2d_config(selected_sample):
    ...
    config_path = os.path.join("output", selected_sample, "bb2d.yaml")
    sample_path = os.path.join("output", selected_sample)

    # Initialize default config
    config = {"view1": None, "view2": None, "good": None, "bad": None}
    ...
    # Auto-select first available files for missing entries
    if not config["view1"]:
        view1_dir = os.path.join(sample_path, "view1", "detections", "visualize")
        if os.path.exists(view1_dir):
            jpg_files = [f for f in os.listdir(view1_dir) if f.lower().endswith(('.jpg', '.jpeg'))]
            if jpg_files:
                config["view1"] = jpg_files[0]
                st.info(f"Auto-selected view1 detection image: {config['view1']}")

    if not config["view2"]:
        view2_dir = os.path.join(sample_path, "view2", "detections", "visualize")
        if os.path.exists(view2_dir):
            jpg_files = [f for f in os.listdir(view2_dir) if f.lower().endswith(('.jpg', '.jpeg'))]
            if jpg_files:
                config["view2"] = jpg_files[0]
                st.info(f"Auto-selected view2 detection image: {config['view2']}")

    if not config["good"]:
        good_dir = os.path.join(sample_path, "mmt", "pred", "class_0")
        if os.path.exists(good_dir):
            nii_files = [f for f in os.listdir(good_dir) if f.lower().endswith(('.nii', '.nii.gz'))]
            if nii_files:
                config["good"] = nii_files[0]
                st.info(f"Auto-selected good segmentation: {config['good']}")

    if not config["bad"]:
        bad_dir = os.path.join(sample_path, "mmt", "pred", "class_0")
        if os.path.exists(bad_dir):
            nii_files = [f for f in os.listdir(bad_dir) if f.lower().endswith(('.nii', '.nii.gz'))]
            # Try to find a different file than the good one if multiple exist
            if len(nii_files) > 1 and config["good"] in nii_files:
                remaining_files = [f for f in nii_files if f != config["good"]]
                if remaining_files:
                    config["bad"] = remaining_files[0]
                    st.info(f"Auto-selected bad segmentation: {config['bad']}")
            elif len(nii_files) == 1:
                config["bad"] = nii_files[0]
                st.info(f"Auto-selected bad segmentation (same as good): {config['bad']}")
...
def calculate_crop_dimensions(sample_path, bb2d_config):
    ...
    # Get paths to the specified images (from detections/visualize for processed images)
    view1_image_path = (
        os.path.join(sample_path, "view1", "detections", "visualize", bb2d_config["view1"]) if bb2d_config["view1"] else None
    )
    view2_image_path = (
        os.path.join(sample_path, "view2", "detections", "visualize", bb2d_config["view2"]) if bb2d_config["view2"] else None
    )
...
def display_detection_images_with_slider(detection_path, view_label):
    ...
    if image_files:
        ...
        selected_idx = st.slider(
            f"Select {view_label} Image", 0, num_images - 1, num_images // 2, key=f"slider_{view_label}"
        )
        ...
        st.image(
            selected_image,
            caption=f"{view_label} Detection (Image {selected_idx+1}/{num_images})",
...
def display_segmentation_point_cloud(segmentation_path, is_full_scan=True):
    ...
    try:
        ...
        plotter.add_points(cloud, scalars="intensity", cmap="viridis", render_points_as_spheres=True, point_size=5)
        plotter.add_axes()
...
def display_segmentation_isosurface(segmentation_path, is_full_scan=True):
    ...
    try:
        ...
        for cls in unique_classes:
            ...
            grid_class = pv.ImageData(dimensions=dims, origin=origin, spacing=spacing)
            ...
            try:
                ...
                plotter.add_mesh(decimated, color=color, opacity=1.0, label=f"Class {cls}", smooth_shading=True)
                has_meshes = True
            except Exception as e:
...
def get_available_samples():
    """Get available samples from output directory."""
    base_path = "output"
    samples = []
...
def display_segmentation_with_2d_slices(
    segmentation_path,
    original_data_path,
    is_full_scan=True,
    quality_label="",
    ...
    try:
        ...
        for cls in unique_classes:
            ...
            try:
                ...
                plotter.add_mesh(decimated, color=color, opacity=0.95, label=f"Class {cls}", smooth_shading=True)
                has_meshes = True
            except Exception as e:
                st.warning(f"Could not create isosurface for class {cls}: {str(e)}")
    ...
    try:
        ...
        with conversion_container:
            ...
            with timer("NII to JPG Conversion"):
                images_path_view1, images_path_view2 = simulate_conversion(sample_path, lambda p: progress.progress(p))
        ...
        with detection_container:
            ...
            with timer("Object Detection"):
                detection_path_view1, detection_path_view2 = simulate_object_detection(
                    sample_path, lambda p: progress.progress(p)
                )
            ...
            try:
                with timer("3D Segmentation"):
                    good_seg_path, bad_seg_path = simulate_3d_segmentation(
                        sample_path, bb2d_config, lambda p: progress.progress(p)
                    )
                ...
                with result:
                    ...
                    if good_seg_file:
                        original_good_data_path = os.path.join(
                            sample_path, "mmt", "img", "class_0", good_seg_file.replace("pred", "img")
                        )
                    else:
                        original_good_data_path = None
                        st.warning(
                            "'good' segmentation file not specified in bb2d.yaml, cannot determine original data path."
                        )

                    if bad_seg_file:
                        original_bad_data_path = os.path.join(
                            sample_path, "mmt", "img", "class_0", bad_seg_file.replace("pred", "img")
                        )
                    else:
                        original_bad_data_path = None
                        st.warning(
                            "'bad' segmentation file not specified in bb2d.yaml, cannot determine original data path."
                        )
                    ...
                    if (
                        original_good_data_path
                        and os.path.exists(original_good_data_path)
                        and original_bad_data_path
                        and os.path.exists(original_bad_data_path)
                    ):
                        try:
                            ...
                            standard_height = min(
                                good_x_slice_shape[0],
                                good_y_slice_shape[0],
                                good_z_slice_shape[0],
                                bad_x_slice_shape[0],
                                bad_y_slice_shape[0],
                                bad_z_slice_shape[0],
                            )
                            standard_width = min(
                                good_x_slice_shape[1],
                                good_y_slice_shape[1],
                                good_z_slice_shape[1],
                                bad_x_slice_shape[1],
                                bad_y_slice_shape[1],
                                bad_z_slice_shape[1],
                            )
                            ...
                            st.info(
                                f"Using standardized dimensions for bump visualizations: {standard_width}x{standard_height} pixels"
                            )
                        except Exception as e:
                            st.warning(
                                f"Error calculating common dimensions: {str(e)}. Will use individual sample dimensions."
                            )
                            ...
                            if bb2d_config.get("good"):
                                ...
                                if good_filename.startswith("pred_") and good_filename.endswith(".nii.gz"):
                                    try:
                                        ...
                                        pass
                            display_segmentation_with_2d_slices(
                                good_seg_path,
                                original_good_data_path,
                                is_full_scan=False,
                                quality_label=good_bump_label,
                    ...
                    with col2:
                        ...
                        if original_bad_data_path and os.path.exists(original_bad_data_path):
                            ...
                            display_segmentation_with_2d_slices(
                                bad_seg_path,
                                original_bad_data_path,
                                is_full_scan=False,
                                quality_label=bad_bump_label,
                                standard_height=standard_height,
                                standard_width=standard_width,
                            )
        ...
        with metrology_container:
            ...
            try:
                with timer("Metrology and Report"):
                    csv_path, pdf_path = simulate_metrology(sample_path, lambda p: progress.progress(p))
...
def process_uploaded_nifti(file_path):
    ...
    try:
        ...
        with segmentation_container:
            st.info(
                "This is a placeholder for the segmentation phase. In a full implementation, this would perform segmentation on the uploaded NIfTI data."
            )
        ...
        with metrology_container:
            st.info(
                "This is a placeholder for the metrology phase. In a full implementation, this would perform measurements on the segmented data."
            )
...
def main():
    ...
    selected_source = st.radio(
        "Select data source:", ["Server Sample", "Upload Your Own Data (In Development)"], horizontal=True
    )
    st.write("---")
    ...
    if selected_source == "Server Sample":
        ...
        if not available_samples:
            st.error("No samples found in the output directory. Please check the server configuration.")
        else:
            # Add blank option as default
            sample_options = [""] + available_samples
            selected_sample = st.selectbox("Select a sample to process", options=sample_options, index=0)
